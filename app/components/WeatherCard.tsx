    ...
    <div className="space-y-2">
      ...
      {weather.rainAccumulationSum > 0 && (
        <div className="space-y-1">
          <p>
            <strong>Rain Avg:</strong> {weather.rainAccumulationAvg.toFixed(2)} in
          </p>
          <p>
            <strong>Rain Max:</strong> {weather.rainAccumulationMax.toFixed(2)} in
          </p>
          <p>
            <strong>Rain Min:</strong> {weather.rainAccumulationMin.toFixed(2)} in
          </p>
          <p>
            <strong>Rain Total:</strong> {weather.rainAccumulationSum.toFixed(2)} in
          </p>
        </div>
      )}
      <p>
      ...
      <p
        className={`text-sm text-stone-950 flex items-center gap-2 ${isWindy ? 'animate-bounce' : ''}`}
      >
        <strong>Wind Speed:</strong> {weather.windSpeed ?? 'N/A'} mph 💨
      </p>

      <p>
        <strong>Wind Gust:</strong> {weather.windGust} mph
      </p>
      <p>
        <strong>Precipitation Probability:</strong> {weather.precipitationProbability}%
      </p>
      <p>
        Day: {getIcon(weather.conditions.day)} {getLabel(weather.conditions.day)}
      </p>
      <p>
      ...
      {nextEmailInHours !== null && nextEmailInHours > 0 && (
        <p className="text-xs text-gray-500">Next email in: {nextEmailInHours.toFixed(1)} hours</p>
      )}
